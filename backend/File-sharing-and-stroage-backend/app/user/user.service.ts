...
export const verifyRefreshToken = async (token: string): Promise<any> => {
  try {
    ...
    // Check if the token exists in the user's document
...
/**
 * Deletes the refresh token from the user's document.
 * @param {string} userId - The user ID whose refresh token should be removed.
 * @returns {Promise<any>} - The updated user document.
 */
export const deleteRefreshToken = async (userId: string): Promise<any> => {
  const user = await UserSchema.findByIdAndUpdate(
    userId,
    { refreshToken: "" },
    { new: true },
