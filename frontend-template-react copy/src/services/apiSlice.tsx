...
export const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    uploadProjectForSale:builder.mutation<void, FormData>({
      query: (formData) => ({
        url: '/projects',  // Replace with your actual endpoint
        method: 'POST',
        body: formData,
      }),
    }),
    ...
    getWalletByUser: builder.query<any, string>({
      query: (userId) => `/wallets/user/${userId}`,
    }),
    createWallet: builder.mutation<any, any>({
      query: (data) => ({
        url: '/wallets',
        method: 'POST',
        body: data,
    ...
    // Mutation to credit the wallet
    creditWallet: builder.mutation({
      query: ({ userId, amount }) => ({///:userId/credit
...
export const {
  ...
  useUpdateMutation,
  useGetUserDataQuery,
  useUploadProjectForSaleMutation,
